...
export default function CreateCustomPackagePage() {
  ...
  const createCustomPackageMutation = useCreateCustomPackage({
    onSuccess: (data) => {
      toast.success('Custom Package proposal created successfully! 📦');
      router.push(`/dashboard/packages`);
    },
    onError: (error) => {
      toast.error(error.message || 'Failed to create custom package proposal');
    },
  });

  // Pre-fill fields if lead is selected
  useEffect(() => {
    if (clientInfo.leadId && leads) {
      const lead = leads.find((l) => l.id === clientInfo.leadId);
      if (lead) {
        setClientInfo((prev) => ({
          ...prev,
          ...
          numberOfAdults: lead.numberOfAdults || prev.numberOfAdults,
          numberOfChildren: lead.numberOfChildren || prev.numberOfChildren,
          customerBudget: lead.budget || prev.customerBudget,
          travelStartDate: lead.checkInDate ? new Date(lead.checkInDate).toISOString().split('T')[0] : prev.travelStartDate,
          travelEndDate: lead.checkOutDate ? new Date(lead.checkOutDate).toISOString().split('T')[0] : prev.travelEndDate,
          specialRequirements: lead.requirements || prev.specialRequirements,
        }));
      }
    }
  }, [clientInfo.leadId, leads]);

  // Adjust itinerary days based on packageInfo.numberOfDays
  useEffect(() => {
  ...
  }, [packageInfo.numberOfDays, packageInfo.destinations]);
  ...
  const handleCreateProposal = async (status: CustomPackageStatus) => {
    if (!validateStep(1) || !validateStep(2) || !validateStep(3) || !validateStep(4)) {
      toast.error('Form contains validation errors. Please review all steps.');
      return;
    }

    const payload: CreateCustomPackageDto = {
      ...
      // Pass pricing additions directly inside proposal
      ...({
        status,
    ...
    await createCustomPackageMutation.mutateAsync(payload);
  };
    ...
    <div className="max-w-6xl mx-auto space-y-8 pb-24" style={{ fontFamily: 'var(--font-outfit), sans-serif' }}>
      ...
      <div className="relative overflow-hidden bg-gradient-to-br from-purple-900 via-indigo-900 to-slate-900 rounded-3xl p-8 shadow-xl text-white">
        ...
        <div className="relative z-10">
          ...
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-indigo-200 text-sm font-semibold uppercase tracking-wider">B2B Custom Proposal Builder</span>
          </div>
          <h1 className="text-3xl sm:text-4xl font-extrabold tracking-tight">Generate Custom Package Quote 🚀</h1>
          <p className="text-indigo-150 text-base mt-2">Craft a bespoke itinerary, estimate costs, and dispatch custom share links instantly.</p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-3xl border border-slate-200/60 shadow-sm p-8">
                    ...
                    <div className="md:col-span-2 flex flex-col gap-1">
                      ...
                      <input
                        ...
                        className={`w-full px-3 py-2 border rounded-xl focus:ring-2 focus:ring-indigo-500/40 focus:border-indigo-550 outline-none text-sm ${
                          errors[`itinerary_${idx}_title`] ? 'border-red-300 bg-red-50' : 'border-slate-200'
                        }`}
                      />
                    </div>
                    ...
                    <div className="md:col-span-3 flex flex-col gap-1">
                      ...
                      <textarea
                        ...
                        className={`w-full px-3 py-2 border rounded-xl focus:ring-2 focus:ring-indigo-500/40 outline-none text-sm resize-none ${
                          errors[`itinerary_${idx}_activities`] ? 'border-red-300 bg-red-50' : 'border-slate-200'
                        }`}
                      />
                    </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="flex flex-col gap-1.5">
                ...
                <input
                  ...
                  className="w-full px-4 py-3 border border-slate-200 rounded-2xl focus:ring-2 focus:ring-indigo-500/40 outline-none text-sm text-slate-800"
                />
              </div>
        ...
        {currentStep === 5 && (
          <div className="space-y-6">
            <h2 className="text-xl font-bold text-slate-900 flex items-center gap-2">
              <ClipboardList className="w-5 h-5 text-indigo-600" /> Review Proposal Details
            </h2>
            ...
            {currentStep < steps.length ? (
              ...
              <>
                <button
                  ...
                  onClick={() => handleCreateProposal(CustomPackageStatus.DRAFT)}
                ...
                <button
                  type="button"
                  disabled={createCustomPackageMutation.isPending}
                  onClick={() => handleCreateProposal(CustomPackageStatus.QUOTE_SENT)}
